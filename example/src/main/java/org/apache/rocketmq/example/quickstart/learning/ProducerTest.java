...
public class ProducerTest {
    ...
    private static final InternalLogger log = InternalLoggerFactory.getLogger(ProducerTest.class);
...
//            sendTransaction("T_MSG[" + i + "]");
    ...
    private static void send(String msg) {
        try {
            Message message = new Message("topic_learning_test", "TagA", msg.getBytes(RemotingHelper.DEFAULT_CHARSET));
    ...
    private static final AtomicInteger transId = new AtomicInteger(1);
    ...
    private static void sendTransaction(String msg) {
        try {
            Message message = new Message(MQConstants.TOPIC, "TagA", msg.getBytes(RemotingHelper.DEFAULT_CHARSET));
            message.setTransactionId(String.valueOf(transId.getAndIncrement()));
            // 发送事务消息
            SendResult sendResult = transactionProducer.sendMessageInTransaction(message, null);
            System.out.printf("发送事务消息:[TID:%s, 最大位点:%s] %s%n", message.getTransactionId(), sendResult.getQueueOffset(), msg);
            // 执行本地事务...
...
//            log.warn("executeLocalTransaction: 消息[{}]的决定是否提交事务, 事务ID:{}", msg.getKeys(), msg.getTransactionId());
        ...
        public LocalTransactionState checkLocalTransaction(MessageExt msg) {
            Integer status = 1;//RandomUtil.randomInt(0, 2);
            System.out.printf("检查本地事务: [TID:%s, 最大位点:%s] %s 消息的事务检查结果:%s%n",
                    msg.getKeys(),
                    msg.getQueueOffset(),
